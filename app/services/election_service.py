...
class ElectionService:
    ...
    def add_voter(db: Session, user_id: str, election_id: str):
        ...
        user = UserRepository.get_by_id(db, resolved_user_id)
        if not user and "@" in resolved_user_id:
            user = UserRepository.get_by_email(db, resolved_user_id)
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User not found. Use the user's email or UUID.",
            )

        resolved_user_id = user.id
        existing = VoterRepository.get_voter(db, resolved_user_id, election_id)
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="User is already registered as a voter for this election",
            )
        return VoterRepository.add_voter(db, user_id=resolved_user_id, election_id=election_id)
